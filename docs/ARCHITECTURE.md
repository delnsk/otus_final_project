...
| --- | --- | --- |
| 4 MCP-инструмента с богатым `description` | задание | Тонкий MCP-слой над сервисами приложения |
...
- **Слой приложения** — сервисы-сценарии (use cases) для 4 инструментов.
...
│  │  ├─ tools.py                # 4 инструмента + их description
...
Показывает сквозной поток для каждого из 4 инструментов: от вызова хост-агентом до
...
чанка), `file_type`, `chunk_id`, время индексации. Используются для цитирования
источников в ответе и для `index_status`.

### 6.3. Поток `index_folder`

`scan(glob)` → `load` → `chunk` → `embed` → `store(ChromaDB)` → обновление
BM25-индекса. Каждый подшаг логируется с промежуточным результатом (кол-во файлов,
...
| e2e | 4 MCP-инструмента | вызовы через MCP, проверка входов/выходов/ошибок |
...
| BM25-индекс в памяти при большом корпусе | персистентность/пересборка при `index_folder`, метрики в `index_status` |
...
| Расхождение описаний инструментов с поведением | содержательные `description`, проверяемые e2e и шагом сдачи 5 |
...
| Фаза | Статус |
| --- | --- |
| 0–13 | Реализовано (2026-06-15): каркас, домен, логи, загрузчики, ChromaDB, retrieval, сервисы, LangGraph, MCP, Log Viewer, Docker-файлы, sample_docs (558 КБ), 54 теста |
